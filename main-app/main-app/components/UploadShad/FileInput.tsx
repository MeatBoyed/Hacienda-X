...
import { handleRejectedFiles } from "./lib/ControllerLayer";
    ...
    <div className="relative flex flex-col w-full gap-6 overflow-hidden">
      ...
      >
        {({ getRootProps, getInputProps, isDragActive }) => (
          ...
          >
            <input {...getInputProps()} />
            <div ref={ref} {...props} className="flex flex-col items-center justify-center gap-4 sm:px-5">
        ...
        <ScrollArea className="h-fit w-full px-3">
          <div className="max-h-48 space-y-4">
            {files?.map((file, index) => (
              <FileInputCard key={index} file={file} onRemove={() => onRemove(index)} customLoader={customLoader} />
            ))}
          </div>
        </ScrollArea>
      ) : null}
...
const FileInputActiveContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
