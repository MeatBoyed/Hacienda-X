...
import { HomePageSearchBar } from "@/components/SearchBar";
import { HeroDict } from "@/global";

export default function Hero({ dict: { heading, subHeading } }: { dict: HeroDict }) {
  return (
    <section id="hero" className="w-full h-[70vh] flex justify-center items-center">
      <div className="flex justify-center items-center w-full flex-col z-10 gap-8 text-white px-4 pt-20 sm:max-w-2xl sm:px-2 sm:pt-0 lg:max-w-4xl">
        <div className="flex justify-center items-center flex-col gap-2">
          <h1 className="text-center scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-5xl">{heading}</h1>
          <p className="text-lg font-semibold text-center text-white">{subHeading}</p>
        </div>
