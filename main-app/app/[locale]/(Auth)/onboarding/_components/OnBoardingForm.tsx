...
async function sendRequest(url: string, { arg }: { arg: { role: "viewer" | "agent" } }) {
  return fetch(url, {
    method: "POST",
    headers: {
...
export default function OnBoardingForm({ userId, firstName }: { userId: string; firstName: string | null }) {
  // Check if User is in DB, if they are - show selected option
  const { data, isLoading } = useSWR<SelectUserResponse>(`/api/user/${userId}`, fetcher, {
  ...
  return (
    <div className="flex justify-center items-center flex-col gap-8">
      <div className="flex justify-center items-center flex-col w-full gap-2">
        ...
        <h3 className="mt-1 text-center text-sm text-muted-foreground">You can change these settings at any time</h3>
      </div>
      {isLoading && (
      ...
      {!isLoading && <CustomerOrAgentSelect isCreated={data?.results} userId={userId} />}
    </div>
  );
}

function CustomerOrAgentSelect({ isCreated, userId }: { isCreated?: $Enums.Role; userId: string }) {
  ...
  const { trigger, isMutating, data } = useSWRMutation("/api/user/updateRole", sendRequest /* options */, {
    onError: () => {
      toast({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: <Link href={toastMessage?.link || ""}>{toastMessage?.message || ""}</Link>,
        duration: 20000,
      });
    },
  });
  ...
  return (
    <div className="flex justify-center items-center gap-4 flex-col w-full sm:flex-row sm:max-w-3xl lg:max-w-5xl">
      ...
      {!isMutating && (
        <>
          <Card
            className={cn("min-w-sm flex w-full justify-center items-center flex-col text-center px-4 py-4 gap-5 h-52")}
            onClick={async () => {
              setToastMessage({
          ...
          >
            ...
            <CardDescription>View properties in your area, and make enquires...</CardDescription>
          </Card>
          <Card
            className={cn(
