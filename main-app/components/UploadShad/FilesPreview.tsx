...
import { useFilesContext } from "./hooks/filesContext";
...
export default function FilesPreview({ children, customLoader }: FilesPreviewProps) {
  ...
  const renderFiles = useMemo(() => {
    ...
    if (files.length > 0) {
      ...
      return <EmptyCard title="No files uploaded" description="Upload some files to see them here" className="w-full" />;
    }
        ...
        <div className="w-full">
          ...
          <DragDropContext onDragEnd={handleDragDrop}>
            <Droppable droppableId="droppable-1" direction={"vertical"} type="group">
              {(provided) => (
                <div className="grid gap-3  w-full" {...provided.droppableProps} ref={provided.innerRef}>
                  {renderFiles}
...
FilesPreview.Head = function FilesPreviewHead({ children }: PropsWithChildren) {
  return <div className="flex justify-center items-start w-full flex-col text-start">{children}</div>;
};
  ...
  return (
    ...
    >
      <AspectRatio ratio={4 / 3}>
        <Image src={fileUrl} alt={`image 1`} fill quality={35} loader={customLoader} className="rounded-md object-cover" />
      </AspectRatio>
