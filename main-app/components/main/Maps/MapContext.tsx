...
export const MapContextProvider: React.FC<{
...
}> = ({ children, focusProperty, properties }) => {
  const [mainProperty, setMainProperty] = useState<PropertyWithAddress>(
    properties.find((p) => p.property_id === focusProperty) || properties[0]
  );
  ...
  return (
    <MapContext.Provider value={{ mainProperty, properties, openProperty }}>
      {children}
    </MapContext.Provider>
  );
};
